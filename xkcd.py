...
for file_name in ['head', 'entry', 'tail']:
    with open('templates/' + file_name + '.html') as f:
        TEMPLATES[file_name] = string.Template(f.read());
...
def update_meta(meta):
    ...
    prev_number = int(meta['num']) - 1 if meta['num'] > 1 else 1
    meta['prev_url'] = str(prev_number) + '.html'
    next_number = int(meta['num']) + 1
    meta['next_url'] = str(next_number) + '.html'
